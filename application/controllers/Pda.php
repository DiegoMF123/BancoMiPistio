...
// si se cumple la funicón para poder guardar los datos, mandamos los respectivos  datos
    if(empty($usuario)){
      ...
      $data["guardar"] = $this->model_pda->guardar($nombrepda,$tipo,$fecha);
    ...
    header("Location: http://192.168.0.11:8888/BancoMiPistio/index.php/pda/nuevo?response=1");
      die();
    }
    // si no se cumple la funcíón if, se quedará en la misma url y no mostrar ningun mensaje y los datos no se guardaran
    else {
      header("Location: http://192.168.0.11:8888/BancoMiPistio/index.php/pda/nuevo");
      die();
      }
...
// si se modifican los datos redireccionará a esta url, si no, no modificará los datos
  header("Location: http://192.168.0.11:8888/BancoMiPistio/index.php/welcome/admin?response=1");
              die();
